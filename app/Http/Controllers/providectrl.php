...
class providectrl extends Controller
{
    ...
    {
    //return "ok";
    	$vs=DB::collection('message')->where('project_status','!=','completed')->get();
    ...
    public function after_pay(Request $req)
    {
      ...
      //$jobname=$req->jobname;
      $projid=$req->pro_id;
      $col3=DB::collection('message')->where('fstatus','ended')->where('payment','2')->update(['project_status'=>'completed']);
    	$col2=DB::collection('provider_payment')->where('pro_id',$projid)->update(['Approve'=>'1']);
    	return "approve";
    }
}
